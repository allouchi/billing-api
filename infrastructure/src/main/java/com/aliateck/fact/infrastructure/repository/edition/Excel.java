...
public class Excel {
     ...
     * @return
     */
    protected static HSSFCellStyle getTitleStyle(HSSFWorkbook wb, short cellColor, short policeColor, short policeSize) {
	HSSFCellStyle titre = wb.createCellStyle();
	titre.setAlignment(HorizontalAlignment.CENTER);
	titre.setVerticalAlignment(VerticalAlignment.CENTER);
	titre.setWrapText(false);
     ...
     */
    protected static HSSFFont getFont(HSSFWorkbook wb, String fontName, short fontSize, short fontColor, boolean bold) {
     ...
     * @param fontColor color of the font eg HSSFiSelectionColors.BLACK.index.
     */
    protected static HSSFFont getTitleFont(HSSFWorkbook wb, String fontName, short fontSize, short fontColor) {
     ...
     * 
     * </br> NB : prefer using {@link #createCell(HSSFRow, short, HSSFCellStyle)} instead.
     * 
     * @param wb the workbook
     ...
     * 
     */
    protected static HSSFCell createCell(HSSFWorkbook wb, HSSFRow row, int column, HSSFCellStyle cellStyle) {
    ...
    /**
     * 
     * Converts a String in acceptable reference name. This method must be used when using a string that contains illegal characters as a reference name for a cell. </br> Illegal characters are (space is a character) : [<b><code>\. ()</code></b>]
     * 
     * @param name
     * @return
     */
    ...
    protected static HSSFCell createCell(HSSFWorkbook wb, HSSFRow row, short column, HSSFCellStyle cellStyle, CellType cellType) {
     ...
     */
    protected static HSSFCellStyle applyBorder(HSSFCellStyle style, BorderStyle thickness, short color) {
	...
	HSSFFont normalFont = getFont(wb, DEFAULT_FONT_NAME, (short) 8, HSSFiSelectionColors.BLACK.getIndex(), false);
     ...
     * @border
     */
    public static void mergeCells(HSSFWorkbook wb, HSSFSheet sheet, int row, int column1, int column2, BorderStyle border) {
	// region
    ...
    /**
     * 
     * Clones the style but NOT the associated font (use {@link #cloneFont(HSSFFont)} for that purpose).
     * 
     * <b>WARNING</b> : do not overuse this function as there is a limit on the number of individual styles that can be present on a single sheet.
     * 
     */
    ...
    public static void setRegionBorders(BorderStyle border, CellRangeAddress region, HSSFSheet sheet, HSSFWorkbook wb) {
	...
	for (int i = region.getFirstRow(); i <= region.getLastRow(); i++) {
	    ...
	    CellUtil.setCellStyleProperty(CellUtil.getCell(row, region.getFirstColumn()), "borderLeft", border);
	    CellUtil.setCellStyleProperty(CellUtil.getCell(row, region.getLastColumn()), "borderRight", border);
	}
     ...
     * @border
     */
    protected static void mergeMultiRowCells(HSSFWorkbook wb, HSSFSheet sheet, int row1, int row2, int column1, int column2, BorderStyle border) {
	// region
    ...
    protected static void copyRange(HSSFWorkbook wb, AreaReference area, HSSFSheet toSheet, CellReference startCell) {
    ...
    /**
     * Copy a range of cells that are in the same sheet. Does not require a sheet name. It handles empty cells by ignoring them during the copy process.
     * 
     * @param area
     * @param sheet
     * @param startCell
     */
    protected static void copyRangeSameSheet(AreaReference area, HSSFSheet sheet, CellReference startCell) {
	CellReference[] crefs = area.getAllReferencedCells();
     ...
     * @return
     */
    public static final HSSFName nameCell(HSSFWorkbook wb, String sheetName, String cellName, int row, int col) {
     ...
     * @return
     */
    public static final HSSFName nameCell(HSSFWorkbook wb, String cellName, CellReference cellReference) {
     ...
     * @throws HSSFUserException
     */
    public static final String area(HSSFWorkbook wb, int startRow, int startCol, int endRow, int endCol) throws HSSFUserException {
	AreaReference areaReference = new AreaReference(new CellReference(startRow, startCol, true, true), new CellReference(endRow, endCol, true, true), SpreadsheetVersion.EXCEL2007);
     ...
     * @return
     */
    public static final HSSFName nameRange(HSSFWorkbook wb, String cellName, AreaReference areaReference) {
	HSSFName namedCell = wb.createName();
	namedCell.setNameName(refName(cellName));
     ...
     * @throws HSSFUserException
     */
    public static final HSSFName nameRange(HSSFWorkbook wb, String cellName, int startRow, int startCol, int endRow, int endCol) throws HSSFUserException {
	AreaReference areaReference = new AreaReference(new CellReference(startRow, startCol, true, true), new CellReference(endRow, endCol, true, true), SpreadsheetVersion.EXCEL2007);
     ...
     * @throws HSSFUserException
     */
    public static final HSSFName nameRange(HSSFWorkbook wb, String cellName, CellReference startCellRef, CellReference endCellRef) throws HSSFUserException {
	AreaReference areaReference = new AreaReference(startCellRef, endCellRef, SpreadsheetVersion.EXCEL2007);
	return nameRange(wb, cellName, areaReference);
    }
	...
	return getCellFromReference(wb, new CellReference(getHSSFName(wb, cellName).getRefersToFormula()));
    }
    ...
    /**
     * 
     * This method should be preferred to {@link HSSFSheet#createRow(int)} as it gets an existing row in the HSSF Model rather than creating a new one for each call. This enables retrieving cells already created.
     * 
     * @param sheet
     * @param lineNb
     * @return
     ...
     * @throws HSSFUserException
     */
    public String formulaAdd(HSSFWorkbook wb, int line1, int col1, int line2, int col2) throws HSSFUserException {
    ...
    /**
     *
     * Returns SUMIF(${ifCol, ifLine1}:${ifCol, ifLine2} , condition, ${column, line1}:${column, line2} )
     *
     * @param wb
     * @param column
     * @param line1
     ...
     * @return
     */
    public String formulaSumIf(HSSFWorkbook wb, int column, int line1, int line2, String ifCondition, int ifCol, int ifLine1, int ifLine2) {
	CellReference start = new CellReference(line1, column, true, true);
	CellReference end = new CellReference(line2, column, true, true);
	...
	return "SUMIF(" + start.formatAsString() + ":" + end.formatAsString() + ",\"" + ifCondition + "\"," + startIf.formatAsString() + ":" + endIf.formatAsString() + ")";
    }
     ...
     * @throws HSSFUserException
     */
    protected String formulaAdd(HSSFWorkbook wb, String cell1, String cell2) throws HSSFUserException {
	return "(" + cell1 + "+" + cell2 + ")";
    }
     ...
     * @throws HSSFUserException
     */
    protected String formulaPercentage(HSSFWorkbook wb, int line1, int col1, int line2, int col2) throws HSSFUserException {
     ...
     * @throws HSSFUserException
     */
    protected String formulaPercentage(HSSFWorkbook wb, String cell1, String cell2) throws HSSFUserException {
	return cell1 + "/" + cell2 + "* 100"; // formulaError(cell1 + "/" + cell2 + "* 100", "0"); // DOES NOT WORK... PB wiht
					      // poi version ?
    }
    ...
    protected String formulaRank(HSSFWorkbook wb, int cellCol, int cellRow, int startCol, int startRow, int endCol, int endRow) throws HSSFUserException {
	return "RANK( " + reference(wb, cellRow, cellCol) + "," + area(wb, startRow, startCol, endRow, endCol) + ", 0 )";
    }
     ...
     * @param titles
     */
    protected void createTitlesForGrid(HSSFWorkbook wb, HSSFSheet sheet, int lineNb, int columnNb, HSSFCellStyle mainTitleStyle, HSSFCellStyle subTitleStyle, String mainTitle, String... titles) {
	HSSFRow row = getOrCreateRow(sheet, lineNb);

	int numberOfColumns = titles.length;

	createCell(wb, row, (short) columnNb, mainTitleStyle).setCellValue(new HSSFRichTextString(mainTitle));
	mergeCells(wb, sheet, (short) lineNb, (short) columnNb, (short) (columnNb + numberOfColumns - 1), BorderStyle.THIN);
	...
	for (int i = 0; i < numberOfColumns; i++) {
	    createCell(wb, row, (short) columnNb++, subTitleStyle).setCellValue(new HSSFRichTextString(titles[i]));
	}
    }
	...
	for (HSSFiSelectionColors color : HSSFiSelectionColors.values()) {
	    ...
	    cellStyle.setFont(getFont(wb, DEFAULT_FONT_NAME, (short) 10, HSSFiSelectionColors.WHITE.getIndex(), true));
	    ...
	    cellStyle.setWrapText(false);
	    cellStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
	    cellStyle.setFont(getFont(wb, DEFAULT_FONT_NAME, (short) 10, HSSFiSelectionColors.BLACK.getIndex(), true));
	    cellStyle.setFillForegroundColor(color.getIndex());
	    cell.setCellStyle(cellStyle);
     ...
     *
     * @param maxChars
     * @return le nom avec au maximum 4 characteres + ".". Par exemple: <br> "TOTOBOULA" -> "TOTO." pour appel avec 4 <br> "VO" -> "VO" pour apple appele avec 4
     */
	...
	HSSFRichTextString formattedCurrency = new HSSFRichTextString(currencyKFormat.format(currency / 1000));
	return formattedCurrency;
    }
	...
	HSSFRichTextString formattedPercent = new HSSFRichTextString(percentFormat.format(percent) + " %");
	...
	if (from != null) {
	    b.append(" du ");
	    b.append(JbbFactory.newJbbDate(from, Locale.FRANCE).formatDateMedium());
	}
	if (to != null) {
	    b.append(" au ");
	    b.append(JbbFactory.newJbbDate(to, Locale.FRANCE).formatDateMedium());
	}
	...
	b = new StringBuffer();
	b.append("Généré le ");
	b.append(JbbFactory.newJbbDate(Locale.FRANCE).formatDateFull());
	b.append(" à ");
	b.append(JbbFactory.newJbbDate(Locale.FRANCE).formatTimeShort());
	cell.setCellValue(new HSSFRichTextString(b.toString()));
