        ...
        try (POIFSFileSystem fs = new POIFSFileSystem(template); HSSFWorkbook wb = new HSSFWorkbook(fs)) {
            ...
            for (Facture facture : factures) {
                if (facture.getDateEncaissement() != null) {
                    ...
                    totalTvaNet += (facture.getMontantTVA() - 30);
                }
            }
            ligneNb++;
            HSSFRow row = getOrCreateRow(sheet1, ligneNb++);
            ...
            createCell(wb, row, 0, applyBorder(normalStyle)).setCellValue("Total TVA NET");
            createCell(wb, row, 1, applyBorder(normalStyle)).setCellValue(totalTvaNet + "€");
