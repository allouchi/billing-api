...
import com.sbatec.fact.infrastructure.repository.edition.commun.HSSFiSelectionColors;
...
public class EditionSuiviFactures {
	 ...
	 * @param fontSize
	 *            size of the font eg 12.
	 * @param fontColor
	 *            color of the font eg HSSFiSelectionColors.BLACK.index.
	 */
	 ...
	 *            the workbook
	 * @param color
	 *            color of the forground for the cell.
	 */
	...
	protected void createStyles(HSSFWorkbook wb) {
		...
		// fonts
		HSSFFont tFont = getTitleFont(wb, DEFAULT_FONT_NAME, (short) 8, HSSFiSelectionColors.BLACK.getIndex());
		tFont.setBold(true);
		HSSFFont normalFont = getTitleFont(wb, DEFAULT_FONT_NAME, (short) 8, HSSFiSelectionColors.BLACK.getIndex());
	...
	private void createTitle(HSSFSheet sheet1, HSSFWorkbook wb) {
		...
		createCell(wb, row, 0, applyBorder(titre))
				.setCellValue(new HSSFRichTextString("Fichier de suivi facturation : " + LocalDate.now().getYear()));
	}
	...
	private void createHeaders(HSSFSheet sheet1, HSSFWorkbook wb) {
		...
		createCell(wb, row, cellIdx++, applyBorder(columnHeaderStyle)).setCellValue(new HSSFRichTextString("Délai"));
		createCell(wb, row, cellIdx++, applyBorder(columnHeaderStyle))
	...
	public void build(List<Facture> factures, InputStream template, String path) throws Exception {
		...
		try (POIFSFileSystem fs = new POIFSFileSystem(template); HSSFWorkbook wb = new HSSFWorkbook(fs)) {
			...
			for (Facture facture : factures) {
				...
				createCell(wb, row, cellIdx++, applyBorder(normalStyle)).setCellValue(
						facture.getNumeroFacture() + " (" + facture.getMoisFacture().substring(0, 3) + ".)");
				createCell(wb, row, cellIdx++, applyBorder(normalStyle)).setCellValue(facture.getPrixTotalHT() + " €");
				createCell(wb, row, cellIdx++, applyBorder(normalStyle)).setCellValue(facture.getPrixTotalTTC() + " €");
				createCell(wb, row, cellIdx++, applyBorder(normalStyle)).setCellValue(facture.getDateFacturation());
				createCell(wb, row, cellIdx++, applyBorder(normalStyle)).setCellValue(facture.getDelaiPaiement());
				createCell(wb, row, cellIdx++, applyBorder(normalStyle)).setCellValue(facture.getDateEcheance());
				createCell(wb, row, cellIdx++, applyBorder(normalStyle)).setCellValue(facture.getNbJourRetard());
				if (facture.getFactureStatus().equalsIgnoreCase("OK")) {
					createCell(wb, row, cellIdx++, applyBorder(statutStyleOk)).setCellValue(facture.getFactureStatus());
					createCell(wb, row, cellIdx++, applyBorder(statutStyleOk))
							.setCellValue(facture.getDateEncaissement());
				} else {
					createCell(wb, row, cellIdx++, applyBorder(statutStyleKo)).setCellValue(facture.getFactureStatus());
					createCell(wb, row, cellIdx++, applyBorder(statutStyleKo))
							.setCellValue(facture.getDateEncaissement());
				}

				createCell(wb, row, cellIdx++, applyBorder(normalStyle)).setCellValue(facture.getFraisRetard());
	 ...
	 *            the workbook
	 * @param thickness
	 *            thickness style of the border eg BorderStyle.THIN
	 * @param color
	 *            color of the border HSSFiSelectionColors.BLACK.index
	 */
